...
- 托盘常驻、单实例、自动 UAC 提权
- **开机自启（v1.8.4）**：首次启动的欢迎界面会显示「开机自动启动」选项并回显当前状态（默认勾选、可当场取消）；安装包的「开机自动启动」任务、欢迎界面、「设置 → 常规」里的开关共用同一个启动项（`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`，卸载时自动清理）
...
| 安装包 | 体积 | 适用场景 |
|---|---|---|
| `SeewoAutoLogin_Setup_v1.8.3.exe` | 约 8MB | 能联网：缺运行时由程序自动下载并静默安装 |
| `SeewoAutoLogin_Setup_v1.8.3_WithWebView2.exe` | 约 210MB | 断网 / 内网机器：安装时离线装好 WebView2，杜绝运行时缺失导致的白屏 |
