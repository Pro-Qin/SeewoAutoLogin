# v5：宏伟 / 电影感（BPM 140，30 拍 + 3 秒长尾 = 约 15.9 秒）
# 在 v4 基础上：① 开头三句疑问句配「秒针式 tick + 重击」的卡点音型
# ② 进结尾前加定音鼓滚奏 ③ 结尾加一层高八度弦乐并把尾音拉到 3 秒
...
BEAT = 60.0 / BPM
BARS = 7.5                     # 30 拍
TAIL = 3.0                     # 尾音：结尾大和弦的 3 秒长衰减（画面同时渐隐，不会显拖沓）
...
def tick(t0, amp=0.3):
    """秒针 / 心跳式点击：给开头三句疑问句提供明确的音乐卡点"""
    s0, n = int(t0 * SR), int(0.09 * SR)
    for i in range(n):
        t = i / SR
        e = math.exp(-t / 0.011)
        v = (math.sin(2 * math.pi * 1180 * t) * 0.55 + math.sin(2 * math.pi * 190 * t) * 0.85) * e * amp
        put(s0 + i, v, v)

def timpani(t0, amp=0.5, freq=73.42):
    """定音鼓单次击打：低频 + 轻微音高下坠 + 中等衰减"""
    s0, n = int(t0 * SR), int(0.34 * SR)
    ph = 0.0
    for i in range(n):
        t = i / SR
        f = freq * (1.0 + 0.07 * math.exp(-t / 0.018))
        ph += 2 * math.pi * f / SR
        e = math.exp(-t / 0.085)
        put(s0 + i, math.sin(ph) * e * amp, math.sin(ph) * e * amp)
...
# ---------- 开头卡点：三句疑问句（0/2/4 拍切换）----------
# 每拍一记 tick（秒针感），奇偶拍强弱交替；疑问句切换点再压一记重击
for i in range(6):
    tick(i * BEAT, 0.34 if i % 2 == 0 else 0.2)
tick(6 * BEAT - BEAT * 0.25, 0.16)      # 第 6 拍前的预告拍
for b in (0, 2, 4):
    bigdrum(b * BEAT + 0.0, 0.95)
    cymbal(b * BEAT, 1.1, 0.2, panv=0.3)
    sub(b * BEAT, 0.5, 73.42, 0.36)
...
# ---------- 定音鼓滚奏：进结尾前约 1 秒，把张力拉满 ----------
ROLL_START, ROLL_END = 23, 25
roll_len = (ROLL_END - ROLL_START) * BEAT
roll_hits = 30
for i in range(roll_hits):
    x = i / (roll_hits - 1)
    t = ROLL_START * BEAT + x * roll_len
    timpani(t, 0.10 + 0.72 * (x ** 1.7), freq=73.42 if i % 4 else 98.0)
...
for f in (146.83, 220.00, 293.66, 369.99, 440.00, 587.33):
    strings(25 * BEAT, 3.4, f, 0.42, attack=0.02, rel=2.8)
# 高八度弦乐层：让结尾更亮、更宏大
for f in (587.33, 880.00, 1174.66, 146.83 * 4):
    strings(25 * BEAT, 3.2, f, 0.24, attack=0.06, rel=2.6)
sub(25 * BEAT, 3.2, 73.42, 0.36)
cymbal(25 * BEAT, 3.2, 0.22)
...
with wave.open('music-v5.wav', 'wb') as w:
...
with open('beats-v5.json', 'w', encoding='utf-8') as f:
...
print('music-v5.wav: %.2f 秒（%d 拍 + %.1fs 尾音）, D 大调宏伟版' % (DUR, 30, TAIL))